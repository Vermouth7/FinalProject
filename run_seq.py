...
from trainer_seq import QuestionAnsweringSeq2SeqTrainer
...
MODEL_PATH='../model_dir_gen/'+model_name
...
def main():
    ...
    def post_processing_function(
        ...
        for example_index, example in enumerate(examples):
            ...
            # print(example_index)
            if example_index in feature_per_example:
        ...
        if training_args.output_dir is not None:
            if not os.path.isdir(training_args.output_dir):
                raise EnvironmentError(f"{training_args.output_dir} is not a directory.")
        ...
        references_dict={}
        for ex in examples:
            references_dict.update({str(ex['id']):ex[answer_column]})
