...
class MyCallback(TrainerCallback):
    def __init__(self, eval_dataset,tokenizer,model,metric,training_args):
        self.examples = eval_dataset
    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        for ex in self.examples:
            references_dict.update({str(ex['id']):ex['answers']})
        ...
        for item in self.examples:
            ...
            predictions[item["id"]]=generated_text
        transformers.utils.logging.set_verbosity_info()
            
        if self.training_args.output_dir is not None:
            if not os.path.isdir(self.training_args.output_dir):
                raise EnvironmentError(f"{self.training_args.output_dir} is not a directory.")

            prefix='eval'
            prediction_file = os.path.join(
                self.training_args.output_dir, "predictions.json" if prefix is None else f"{prefix}_predictions.json"
            )
        ...
        for key in list(metrics.keys()):
                if not key.startswith(f"{'eval'}_"):
                    metrics[f"{'eval'}_{key}"] = metrics.pop(key)
...
def main():
    ...
    MODEL_PATH='/work/m22oc/m22oc/s2338291/FinalProj/model_dir_gen/'+model_name
    ...
    if training_args.do_eval:
        ...
        metric = evaluate.load("../metric/accuracy.py")
    ...
    metric_custom=evaluate.combine([
        evaluate.load('../metric/squad_gen.py'),
        evaluate.load('../metric/tweetqa_eval_gen.py')
    ])
    ...
    trainer = Trainer(
        ...
        callbacks=[MyCallback(eval_dataset=raw_datasets['validation'],tokenizer=tokenizer,model=model,metric=metric_custom,training_args=training_args)],
    )
